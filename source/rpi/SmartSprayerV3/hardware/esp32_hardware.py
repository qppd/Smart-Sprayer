...
class ESP32Hardware(HardwareInterface):
    ...
    def __init__(self, port='/dev/ttyUSB0', baudrate=9600, timeout=1):
        ...
        # ── Serial I/O lock ────────────────────────────────────────────────────
        # Prevents concurrent threads (dashboard, notifications, scheduler) from
        # interleaving writes/reads on the same UART stream, which causes the ESP32
        # to receive garbled commands and miss relay activations.
    ...
    def _send_command(self, command, extra_timeout=0.0):
        """Send command to ESP32 and return response.

        All access is serialised through self._io_lock so that concurrent threads
        (dashboard polling, notifications, scheduler spray) never interleave bytes
        on the shared UART stream.

        Args:
            command: Plain-text command string (no trailing newline).
            extra_timeout: Added on top of self.timeout for long-running commands
                           (e.g. spray where we wait for completion ACK).
        """
        ...
        with self._io_lock:
            try:
                # Flush any stale bytes left over from a previous interaction.
                ...
                # Read response (some commands return multi-line output).
                response_lines = []
                deadline = time.monotonic() + max(0.5, float(self.timeout)) + extra_timeout
                last_data_time = None
                ...
                # Debug print for status commands only.
    ...
    def _send_framed_command(self, command: str, data: str = "") -> str:
        ...
        Serialised through self._io_lock (same as _send_command) so framed
        level-reads cannot collide with plain relay/spray commands.
        """
        if not self.connected or not self.serial_connection:
    ...
    def spray(self, relay_num, duration_seconds, volume_ml):
        """Execute spray operation on ESP32 and wait for real completion ACK.

        With the non-blocking ESP32 firmware the sequence is:
          1. RPi sends  spray_<relay>_<duration>_<volume>
          2. ESP32 replies immediately: ACK:SPRAY_STARTED
             (relay is already ON at this point)
          3. After <duration> seconds ESP32 replies: ACK:SPRAY_DONE
             (relay is now OFF, SMS/buzzer may still be running on ESP32
              but that does not affect the relay)

        We wait up to (duration + GSM_SLACK) seconds for ACK:SPRAY_DONE
        before declaring the operation complete.  If the firmware is the old
        blocking version the response will contain '[SPRAY] Completed' instead
        and we handle both.
        ...
        Returns:
            True on confirmed completion, False on timeout/error.
        """
        if not self.connected:
            print("[ESP32] Not connected. Cannot execute spray.")
            return False

        # Extra slack for SMS send-to-all (up to 10 recipients × 15 s each).
        # The non-blocking firmware fires SMS *after* relay OFF so this only
        # matters for the old firmware fallback.
        GSM_SLACK = 30  # seconds

        command = f"spray_{relay_num}_{int(duration_seconds)}_{int(volume_ml)}"
        # Use extra_timeout so _send_command keeps reading until we get the
        # completion ACK.  The lock inside _send_command prevents any polling
        # thread from interleaving while we wait.
        extra = float(duration_seconds) + GSM_SLACK
        response = self._send_command(command, extra_timeout=extra)

        if response:
            completed = (
                "ACK:SPRAY_DONE" in response           # new non-blocking firmware
                or "[SPRAY] Completed" in response      # old blocking firmware
            )
            if completed:
                print(f"[ESP32] Spray CONFIRMED: Relay {relay_num}, {duration_seconds}s, {volume_ml}mL")
                return True
            else:
                print(f"[ESP32] Spray response (no completion marker): {response}")
                return False
        else:
            print(f"[ESP32] Spray TIMEOUT after {duration_seconds + GSM_SLACK}s — no ACK received")
            return False
