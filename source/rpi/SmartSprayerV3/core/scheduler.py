    ...
    def _execute_schedule(self, schedule: Dict):
        ...
        try:
            ...
            if self.hardware:
                ...
                def spray_task():
                    try:
                        success = self.hardware.spray(relay_num, spray_duration, volume_ml)
                        if success:
                            # Mark as completed only after confirmed ACK from ESP32
                            ...
                            self.logger.log_error(
                                f"Spray {schedule['id']} timed out or got no ACK from ESP32"
                            )
                            self.data_store.update_schedule(schedule['id'], {
                                'status': 'failed',
                                'error': 'No ACK received from ESP32'
                            })
                    except Exception as e:
